...
app.post('/api/election-initialize/', async function (req, res) {
    try {
        ...
        if(responseData.length == 0)
        {
            ...
            console.log('aaa');
            await contract.submitTransaction('updateElectionObj', req.body.electionID , req.body.created_at);
            console.log('bbb');
            console.log('Transaction has been submitted');
            res.send('Transaction has been submitted');
        }
        ...
        {
            console.log('xxx');
            await contract.submitTransaction('setStartElectionBySameIDObject',responseData[0].Key, req.body.electionID , req.body.created_at);
            console.log('aaa');
            await contract.submitTransaction('updateElectionObj', req.body.electionID , req.body.created_at);
            console.log('bbb');
            console.log('Transaction has been submitted');
        ...
        else
        {
            ...
            res.send('Election Already Started');
        }
...
app.post('/api/election-end/', async function (req, res) {
    try {
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
        // Create a new file system based wallet for managing identities.
        ...
        var VoterData = aaryData.filter(function (data) {
        ...
        if(responseData.length != 0)
        {
            ...
            await contract.submitTransaction('endElectionObj', req.body.electionID , req.body.isStarted, req.body.endTime);
            await contract.submitTransaction('setEndElectionByID', responseData[0].Key);

            if(VoterData.length != 0)
            {   
                // console.log

                VoterData.forEach(async  element  =>  {

                  const data =  JSON.stringify(element)
                  const ffff = JSON.parse(data);
                    await contract.submitTransaction('getVotedRemove', ffff.Key);
                });
            }
